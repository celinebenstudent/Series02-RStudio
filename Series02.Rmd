...
Are snorers fatter?
    ...
    Result: Mean weight = 90.3 kg (snorers) vs 90.5 kg (non-snorers).

    Answer: No, snorers are not fatter. Their average weight is almost the same.

Do snorers drink or smoke more?

    Alcohol: Snorers drink more (mean = 4.0 units) than non-snorers (2.4).

    Smoking: Fewer snorers smoke (57%) than non-snorers (68%).

    Answer: Snorers drink more, but they actually smoke less.

Are men fatter?

    Mean weight: Men = 90.8 kg, Women = 89.3 kg.

    Answer: Yes, men are slightly fatter on average.

Do women smoke less?

    Smoking rate: Men = 72%, Women = 40%.

    Answer: Yes, women smoke less than men.

From a more general perspective:

Are there any correlations between variables?

    Strong positive correlation: Weight ↔ Height (r = 0.93).

    Weak correlation: Weight with alcohol (r = 0.09, very small).

    Answer: Yes, mainly between weight and height. Other correlations are weak.
...
NOTE: These questions are more advanced, and I don’t want you to address them from a technical standpoint or provide code. But they give you an idea of what you can do with data mining techniques… provided you know why you want to do it. I invite you to think about the perspective these questions open up and, if you feel ready to do so, to write a short paragraph about it.
    ...
    We can use regression to predict health outcomes from risk factors, or clustering to discover hidden groups of patients with similar profiles. These methods don’t just give numbers — they help us see patterns, anticipate problems, and design better interventions. The key is always to connect the analysis to a meaningful goal, so the statistics serve a purpose and not just curiosity.
